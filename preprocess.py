...
def get_data(inputs_file_path, labels_file_path, num_examples):
    ...
    # TODO: Load inputs and label
    with gzip.open(inputs_file_path, 'rb') as f:
      content = f.read()


      data = np.frombuffer(content, count=784 * num_examples, dtype=np.uint8, offset=16)
      data = np.float32(data) / np.float32(255)
      data.resize(num_examples, 784)

    # TODO: Normalize inputs

    with gzip.open(labels_file_path, 'rb') as g:
      labels = g.read()    
      labels_data = np.frombuffer(labels, dtype=np.int8, count=-1, offset=8)
